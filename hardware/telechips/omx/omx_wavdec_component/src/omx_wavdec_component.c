...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
	...
	err = omx_wavdec_component_Constructor(openmaxStandComp,cCompontName);
...
OMX_ERRORTYPE omx_wavdec_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName) 
{
	...
	*/
	...
	omx_wavdec_component_Private->cdk_core.m_iAudioProcessMode = 2; /* decoded pcm mode */

	omx_wavdec_component_Private->cdk_core.m_psCallback = &(omx_wavdec_component_Private->callback_func);
	omx_wavdec_component_Private->cdk_core.m_psCallback->m_pfMalloc   = (void* (*) ( unsigned int ))malloc;
	omx_wavdec_component_Private->cdk_core.m_psCallback->m_pfRealloc  = (void* (*) ( void*, unsigned int ))realloc;
	omx_wavdec_component_Private->cdk_core.m_psCallback->m_pfFree	  = (void  (*) ( void* ))free;
	omx_wavdec_component_Private->cdk_core.m_psCallback->m_pfMemcpy   = (void* (*) ( void*, const void*, unsigned int ))memcpy;
	omx_wavdec_component_Private->cdk_core.m_psCallback->m_pfMemmove  = (void* (*) ( void*, const void*, unsigned int ))memmove;
	omx_wavdec_component_Private->cdk_core.m_psCallback->m_pfMemset   = (void  (*) ( void*, int, unsigned int ))memset;
  
	omx_wavdec_component_Private->iAdecType = AUDIO_ID_WAV;
	omx_wavdec_component_Private->iCtype = CONTAINER_TYPE_MPG;
	omx_wavdec_component_Private->cb_function = TCC_WAV_DEC;

	if (omx_wavdec_component_Private->pRest == NULL)
	{
		omx_wavdec_component_Private->pRest = (OMX_U8*)malloc(DEFAULT_OUT_BUFFER_SIZE);
	}
