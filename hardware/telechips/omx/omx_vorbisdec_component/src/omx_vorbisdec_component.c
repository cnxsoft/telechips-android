...
OMX_ERRORTYPE OMX_ComponentInit(OMX_HANDLETYPE openmaxStandComp, OMX_STRING cCompontName)
{
  OMX_ERRORTYPE err = OMX_ErrorNone;
	...
	err = omx_vorbisdec_component_Constructor(openmaxStandComp,cCompontName);
...
//	setHeader(&omx_vorbisdec_component_Private->pAudioVorbis, sizeof(OMX_AUDIO_PARAM_VORBISTYPE));
	...
	omx_vorbisdec_component_Private->cdk_core.m_iAudioProcessMode = 2; /* decoded pcm mode */

	omx_vorbisdec_component_Private->cdk_core.m_psCallback = &(omx_vorbisdec_component_Private->callback_func);
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfMalloc   = (void* (*) ( unsigned int ))malloc;
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfRealloc  = (void* (*) ( void*, unsigned int ))realloc;
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfFree	  = (void  (*) ( void* ))free;
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfMemcpy   = (void* (*) ( void*, const void*, unsigned int ))memcpy;
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfMemmove  = (void* (*) ( void*, const void*, unsigned int ))memmove;
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfMemset   = (void  (*) ( void*, int, unsigned int ))memset;
	omx_vorbisdec_component_Private->cdk_core.m_psCallback->m_pfCalloc   = (void* (*) ( unsigned int, unsigned int))calloc;
 
	omx_vorbisdec_component_Private->iAdecType = AUDIO_ID_VORBIS;
	omx_vorbisdec_component_Private->iCtype = CONTAINER_TYPE_MKV;
	omx_vorbisdec_component_Private->cb_function = TCC_VORBIS_DEC;

	if (omx_vorbisdec_component_Private->pRest == NULL)
	{
		omx_vorbisdec_component_Private->pRest = (OMX_U8*)malloc(DEFAULT_OUT_BUFFER_SIZE);
	}
