/**
  ...
  @file omx_mp2dec_component.c

  This component implement Mpeg1-LayerII decoder.
...
OMX_ERRORTYPE omx_mp2dec_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp, OMX_STRING cComponentName) 
{
	...
	*/
	...
	omx_mp2dec_component_Private->sPortTypesParam[OMX_PortDomainAudio].nStartPortNumber = 0;
  	omx_mp2dec_component_Private->sPortTypesParam[OMX_PortDomainAudio].nPorts = 2;
	...
	base_audio_port_Constructor(openmaxStandComp, &omx_mp2dec_component_Private->ports[1], 1, OMX_FALSE); // output
	...
	outPort = (omx_base_audio_PortType *) omx_mp2dec_component_Private->ports[OMX_BASE_FILTER_OUTPUTPORT_INDEX];
    ...
    omx_mp2dec_component_Private->pAudioMp2.nSampleRate = 44100;
	...
	setHeader(&omx_mp2dec_component_Private->pAudioPcmMode, sizeof(OMX_AUDIO_PARAM_PCMMODETYPE));
	omx_mp2dec_component_Private->pAudioPcmMode.nPortIndex = 1;
	omx_mp2dec_component_Private->pAudioPcmMode.nChannels = 2;
	omx_mp2dec_component_Private->pAudioPcmMode.eNumData = OMX_NumericalDataSigned;
	omx_mp2dec_component_Private->pAudioPcmMode.eEndian = OMX_EndianLittle;
	omx_mp2dec_component_Private->pAudioPcmMode.bInterleaved = OMX_TRUE;
	omx_mp2dec_component_Private->pAudioPcmMode.nBitPerSample = 16;
	omx_mp2dec_component_Private->pAudioPcmMode.nSamplingRate = 44100;
	omx_mp2dec_component_Private->pAudioPcmMode.ePCMMode = OMX_AUDIO_PCMModeLinear;
	omx_mp2dec_component_Private->pAudioPcmMode.eChannelMapping[0] = OMX_AUDIO_ChannelLF;
	omx_mp2dec_component_Private->pAudioPcmMode.eChannelMapping[1] = OMX_AUDIO_ChannelRF;

	/** now it's time to know the audio coding type of the component */
	if(!strcmp(cComponentName, AUDIO_DEC_MP2_NAME))  
	{   
		 omx_mp2dec_component_Private->audio_coding_type = OMX_AUDIO_CodingMP2;
	} 
	else if (!strcmp(cComponentName, AUDIO_DEC_BASE_NAME)) 
	{
		omx_mp2dec_component_Private->audio_coding_type = OMX_AUDIO_CodingUnused;
	}
	else  
	{
	...
	omx_mp2dec_component_Private->decode_ready = OMX_FALSE;	
	
	memset(&omx_mp2dec_component_Private->cdk_core, 0x00, sizeof(cdk_core_t));
	memset(&omx_mp2dec_component_Private->cdmx_info, 0x00, sizeof(cdmx_info_t));
	memset(&omx_mp2dec_component_Private->cdmx_out, 0x00, sizeof(cdmx_output_t));
	
	omx_mp2dec_component_Private->cdk_core.m_iAudioProcessMode = 2; /* decoded pcm mode */

	omx_mp2dec_component_Private->cdk_core.m_psCallback = &(omx_mp2dec_component_Private->callback_func);
	omx_mp2dec_component_Private->cdk_core.m_psCallback->m_pfMalloc   = (void* (*) ( unsigned int ))malloc;
	omx_mp2dec_component_Private->cdk_core.m_psCallback->m_pfRealloc  = (void* (*) ( void*, unsigned int ))realloc;
	omx_mp2dec_component_Private->cdk_core.m_psCallback->m_pfFree	  = (void  (*) ( void* ))free;
	omx_mp2dec_component_Private->cdk_core.m_psCallback->m_pfMemcpy   = (void* (*) ( void*, const void*, unsigned int ))memcpy;
	omx_mp2dec_component_Private->cdk_core.m_psCallback->m_pfMemmove  = (void* (*) ( void*, const void*, unsigned int ))memmove;
	omx_mp2dec_component_Private->cdk_core.m_psCallback->m_pfMemset   = (void  (*) ( void*, int, unsigned int ))memset;
  
	omx_mp2dec_component_Private->iAdecType = AUDIO_ID_MP2;
	omx_mp2dec_component_Private->iCtype = CONTAINER_TYPE_MPG;
	omx_mp2dec_component_Private->cb_function = TCC_MP2_DEC;

	if (omx_mp2dec_component_Private->pRest == NULL)
	{
		omx_mp2dec_component_Private->pRest = (OMX_U8*)malloc(DEFAULT_OUT_BUFFER_SIZE);
	}
