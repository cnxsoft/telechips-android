...
#define DBug_MSG(msg...)	if (debug) { LOGD( ": " msg);/* sleep(1);*/}
#else
#define DBug_MSG(msg...)	printf(msg)
//#define USE_THREAD_MANAGEMENT
#endif
...
OMX_ERRORTYPE omx_base_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
  ...
  if (openmaxStandComp->pComponentPrivate) {
    ...
    omx_base_component_Private = TCC_calloc(1,sizeof(omx_base_component_PrivateType));
  ...
  omx_base_component_Private->name = TCC_calloc(1,OMX_MAX_STRINGNAME_SIZE);
  ...
  if(!omx_base_component_Private->flush_all_condition) {
    omx_base_component_Private->flush_all_condition = TCC_calloc(1,sizeof(tsem_t));
    tsem_init(omx_base_component_Private->flush_all_condition, 0);
  }

  if(!omx_base_component_Private->flush_condition) {
    omx_base_component_Private->flush_condition = TCC_calloc(1,sizeof(tsem_t));
...
*/
  ...
  ret |= pthread_attr_setschedpolicy(&tattr,THREAD_SCHED_TYPE);
  
  /* safe to get existing scheduling param */
  ret |= pthread_attr_getschedparam (&tattr, &param);
  
  //printf("%s thread priority changed %d ->", omx_base_component_Private->name, param.__sched_priority);  
  
  /* set the priority; others are unchanged */
#if 0
  if( !strcmp(omx_base_component_Private->name, VIDEO_SINK_NAME) || !strcmp(omx_base_component_Private->name, AUDIO_SINK_NAME) ){
	param.__sched_priority = SINK_COMP_PRIO;
  }else{
	param.__sched_priority = OTHER_COMP_PRIO;	
  }
#else
...
OMX_ERRORTYPE omx_base_component_Destructor(OMX_COMPONENTTYPE *openmaxStandComp) {
  ...
  if(omx_base_component_Private->messageQueue) {
    ...
    TCC_free(omx_base_component_Private->messageQueue);
  ...
  if(omx_base_component_Private->bMgmtSem){
    ...
    TCC_free(omx_base_component_Private->bMgmtSem);
  ...
  if(omx_base_component_Private->flush_all_condition){
    ...
    TCC_free(omx_base_component_Private->flush_all_condition);
    omx_base_component_Private->flush_all_condition=NULL;
  }

  if(omx_base_component_Private->flush_condition){
    tsem_deinit(omx_base_component_Private->flush_condition);
    TCC_free(omx_base_component_Private->flush_condition);
...
OMX_ERRORTYPE omx_base_component_ComponentDeInit(
  ...
  TCC_free(openmaxStandComp->pComponentPrivate);
      ...
      for(j = 0; j < NUM_DOMAINS; j++) {
        for(i = omx_base_component_Private->sPortTypesParam[j].nStartPortNumber;
          ...
          if(pPort->pInternalBufferStorage != NULL) {
            TCC_free(pPort->pInternalBufferStorage);
            pPort->pInternalBufferStorage=NULL;
          }
          
          if(pPort->bBufferStateAllocated != NULL) {
            TCC_free(pPort->bBufferStateAllocated);
...
OMX_ERRORTYPE omx_base_component_GetParameter(
  ...
  case OMX_IndexVendorCompPropTunnelFlags:
    ...
    break;
  default:
    DEBUG(DEB_LEV_ERR, "OMX_ErrorUnsupportedIndex %x\n", nParamIndex);   	
...
OMX_ERRORTYPE omx_base_component_SetParameter(
  ...
  case OMX_IndexParamPortDefinition: 
	...
	pPortParam->format.video.nBitrate      		= pPortDef->format.video.nBitrate;
        break;
  ...
  default:
	...
	if(err != OMX_ErrorNone)
		LOGE("ERROR %s(%s) :: nParamIndex = 0x%x, error(0x%x)", __func__, omx_base_component_Private->name, nParamIndex, err);
...
OMX_ERRORTYPE omx_base_component_SendCommand(
  ...
  message = TCC_calloc(1,sizeof(internalRequestMessageType));
    ...
    if ((nParam == OMX_StateIdle) && (omx_base_component_Private->state == OMX_StateLoaded)) {
      ...
      for(j = 0; j < NUM_DOMAINS; j++) {
        for(i = omx_base_component_Private->sPortTypesParam[j].nStartPortNumber;
          ...
          if(pPort->pInternalBufferStorage == NULL) {
            pPort->pInternalBufferStorage = TCC_calloc(pPort->sPortParam.nBufferCountActual,sizeof(OMX_BUFFERHEADERTYPE *));
          }           

          if(pPort->bBufferStateAllocated == NULL) {
            pPort->bBufferStateAllocated = TCC_calloc(pPort->sPortParam.nBufferCountActual,sizeof(BUFFER_STATUS_FLAG));
          } 
...
#ifdef HAVE_ANDROID_OS
OMX_ERRORTYPE omx_base_component_AllocateBuffer2(
            OMX_IN OMX_HANDLETYPE hComponent,
            OMX_INOUT OMX_BUFFERHEADERTYPE** ppBuffer,
            OMX_IN OMX_U32 nPortIndex,
            OMX_IN OMX_PTR pAppPrivate,
...
OMX_ERRORTYPE omx_base_component_UseBuffer2(
            OMX_IN OMX_HANDLETYPE hComponent,
            OMX_INOUT OMX_BUFFERHEADERTYPE** ppBufferHdr,
            OMX_IN OMX_U32 nPortIndex,
            OMX_IN OMX_PTR pAppPrivate,
...
OMX_ERRORTYPE omx_base_component_FreeBuffer2(   
            OMX_IN  OMX_HANDLETYPE hComponent,
            OMX_IN  OMX_U32 nPortIndex,
            OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer) {
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate;
