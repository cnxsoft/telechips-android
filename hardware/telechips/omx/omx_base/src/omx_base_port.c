...
OMX_ERRORTYPE base_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
  ...
  if (!(*openmaxStandPort)) {
    *openmaxStandPort = TCC_calloc(1,sizeof (omx_base_PortType));
  }
  ...
  if((*openmaxStandPort)->pAllocSem==NULL) {
    (*openmaxStandPort)->pAllocSem = TCC_calloc(1,sizeof(tsem_t));
    if((*openmaxStandPort)->pAllocSem==NULL) {
      return OMX_ErrorInsufficientResources;
    }
  ...
  if(!(*openmaxStandPort)->pBufferQueue) {
    (*openmaxStandPort)->pBufferQueue = TCC_calloc(1,sizeof(queue_t));
  ...
  if(!(*openmaxStandPort)->pBufferSem) {
    (*openmaxStandPort)->pBufferSem = TCC_calloc(1,sizeof(tsem_t));
...
#ifdef HAVE_ANDROID_OS
  (*openmaxStandPort)->Port_AllocateBuffer2 = &base_port_AllocateBuffer2;
  (*openmaxStandPort)->Port_UseBuffer2 = &base_port_UseBuffer2;
  (*openmaxStandPort)->Port_FreeBuffer2 = &base_port_FreeBuffer2;
#endif
...
OMX_ERRORTYPE base_port_AllocateBuffer(
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
      openmaxStandPort->pInternalBufferStorage[i] = TCC_calloc(1,sizeof(OMX_BUFFERHEADERTYPE));
      ...
      /* allocate the buffer */
      openmaxStandPort->pInternalBufferStorage[i]->pBuffer = TCC_calloc(1,nSizeBytes);
...
//LOGI("alloc[%d] :: type[%d] header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], *pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
	  ...
	  *ppBufferHdr = openmaxStandPort->pInternalBufferStorage[i];
      ...
      returnBufferHeader = TCC_calloc(1,sizeof(OMX_BUFFERHEADERTYPE));
...
OMX_ERRORTYPE base_port_FreeBuffer(
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] & (BUFFER_ASSIGNED | BUFFER_ALLOCATED)) {
		if(pBuffer == openmaxStandPort->pInternalBufferStorage[i]) //ZzaU :: to arrange free-order
		{
			//LOGD("free[%d] : type[%d], header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
			...
			if (openmaxStandPort->bBufferStateAllocated[i] & BUFFER_ALLOCATED) {
				if(openmaxStandPort->pInternalBufferStorage[i]->pBuffer){
					...
					TCC_free(openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
			...
			if(openmaxStandPort->bBufferStateAllocated[i] & HEADER_ALLOCATED) {
				TCC_free(openmaxStandPort->pInternalBufferStorage[i]);
...
OMX_ERRORTYPE base_port_AllocateTunnelBuffer(omx_base_PortType *openmaxStandPort,OMX_IN OMX_U32 nPortIndex,OMX_IN OMX_U32 nSizeBytes)
{
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] == BUFFER_FREE) {
      pBuffer = TCC_calloc(1,nSizeBytes);
      if(pBuffer==NULL) {
        return OMX_ErrorInsufficientResources;
      }
...
#ifdef HAVE_ANDROID_OS
OMX_ERRORTYPE base_port_AllocateBuffer2(
  omx_base_PortType *openmaxStandPort,
  OMX_BUFFERHEADERTYPE** pBuffer,
  OMX_U32 nPortIndex,
  OMX_PTR pAppPrivate,
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
	...
	//  	  	LOGE("%s %d End\n",__func__,__LINE__);
//LOGI("alloc2[%d] :: type[%d] header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], *pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
 ...
 * bIsFullOfBuffers becomes equal to OMX_TRUE
 */
OMX_ERRORTYPE base_port_UseBuffer2(
  omx_base_PortType *openmaxStandPort,
  OMX_BUFFERHEADERTYPE** ppBufferHdr,
  OMX_U32 nPortIndex,
  OMX_PTR pAppPrivate,
 ...
 * When all the bufers are done, the variable bIsEmptyOfBuffers is set to OMX_TRUE
 */
OMX_ERRORTYPE base_port_FreeBuffer2(
  omx_base_PortType *openmaxStandPort,
  OMX_U32 nPortIndex,
  OMX_BUFFERHEADERTYPE* pBuffer) {
  ...
  for(i=0; i < openmaxStandPort->sPortParam.nBufferCountActual; i++){
    if (openmaxStandPort->bBufferStateAllocated[i] & (BUFFER_ASSIGNED | BUFFER_ALLOCATED)) {
		if(pBuffer == openmaxStandPort->pInternalBufferStorage[i]) //ZzaU :: to arrange free-order
		{		
//			LOGD("free2[%d] :: type[%d] header(0x%x), pBuffer(0x%x - 0x%x)", i, openmaxStandPort->bBufferStateAllocated[i], openmaxStandPort->pInternalBufferStorage[i], pBuffer, openmaxStandPort->pInternalBufferStorage[i]->pBuffer);
